...
import { z } from "zod";
import { dramaPaths, jsonResult, readReference, readTextFile, resolveProjectRoot, textResult, } from "../lib/paths.js";
...
export async function evaluateScript(args) {
    ...
    let brief = null;
    if (args.episode) {
        const briefPath = path.join(dramaPaths(root).briefs, `episode_${String(args.episode).padStart(3, "0")}.json`);
        try {
    ...
    const estimatedOverall = Math.min(100, automatedBase +
    ...
    const report = {
        ...
        agent_evaluation_task: {
            instruction: "请阅读 evaluation_rubric（资源 drama://evaluation-rubric），结合 automated_assessment，输出完整 JSON 评分报告",
...
export async function validateScript(args) {
...
export async function getScriptTemplate(args) {
