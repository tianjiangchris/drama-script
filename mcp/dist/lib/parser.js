...
export function parseScenes(text) {
    ...
    for (const chunk of chunks) {
        ...
        for (const line of lines) {
            if (META_TIME.test(line))
                scene.time = line.replace(/^时间[：:]\s*/, "");
...
export function analyzeScript(text) {
    ...
    const avgLineLength = dialogueLineCount > 0 ? Math.round(dialogueChars / dialogueLineCount) : 0;
    const formatIssues = [];
    ...
    for (const scene of scenes) {
        if (!scene.time)
            formatIssues.push(`场次 ${scene.number} 缺少「时间」`);
        if (!scene.location)
            formatIssues.push(`场次 ${scene.number} 缺少「地点」`);
        if (scene.characters.length === 0)
            formatIssues.push(`场次 ${scene.number} 缺少「人物」`);
    }
    if (totalChars < 400)
        formatIssues.push("字数偏少（<400），可能撑不满 1 分钟");
    if (totalChars > 900)
        formatIssues.push("字数偏多（>900），可能超过 2 分钟");
    if (scenes.length > 5)
        formatIssues.push("场次数偏多（>5），节奏可能碎");
...
export function extractNovelBeats(text, maxBeats = 12) {
