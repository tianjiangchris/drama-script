...
import { z } from "zod";
import { dramaPaths, jsonResult, loadConfig, readTextFile, resolveProjectRoot, saveConfig, textResult, writeTextFile, } from "../lib/paths.js";
...
export async function prepareAdaptation(args) {
    const root = resolveProjectRoot(args.project_root);
    const paths = dramaPaths(root);
    ...
    const briefDoc = {
        ...
        agent_task: "请根据以上 brief 与 extracted_beats，将源材料改编为竖屏短剧剧本。写完后调用 evaluate_script 做评估。",
    };
    const briefPath = path.join(paths.briefs, `episode_${String(args.episode).padStart(3, "0")}.json`);
...
export async function saveScript(args) {
    const root = resolveProjectRoot(args.project_root);
    const paths = dramaPaths(root);
    const filePath = path.join(paths.scripts, `episode_${String(args.episode).padStart(3, "0")}.md`);
