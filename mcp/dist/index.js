...
import { evaluateScript, evaluateScriptSchema, getScriptTemplate, getScriptTemplateSchema, validateScript, validateScriptSchema, } from "./tools/evaluate.js";
import { initProject, initProjectSchema, listEpisodes, listEpisodesSchema, prepareAdaptation, prepareAdaptationSchema, readSource, readSourceSchema, saveScript, saveScriptSchema, } from "./tools/project.js";
const server = new McpServer({
    name: "drama-script",
    version: "1.0.0",
});
server.registerTool("init_project", {
    description: "初始化短剧改编项目，创建 source/、scripts/、.drama-script/ 目录结构",
    inputSchema: initProjectSchema,
}, async (args) => initProject(args));
server.registerTool("prepare_adaptation", {
    description: "读取小说章节，提取情节点，生成分集改编 brief，供 Agent 写竖屏短剧剧本",
...
server.registerTool("evaluate_script", {
    description: "评估剧本：返回自动化指标 + 评估量表 + Agent 需完成的 qualitative 评分任务",
    inputSchema: evaluateScriptSchema,
}, async (args) => evaluateScript(args));
server.registerResource("script-format", "drama://script-format", {
...
server.registerResource("adaptation-guide", "drama://adaptation-guide", {
...
server.registerResource("evaluation-rubric", "drama://evaluation-rubric", {
